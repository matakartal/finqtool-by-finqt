              ...
              <CardContent className="flex flex-col px-6 py-5 bg-muted/40">
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <div className="text-xs text-neutral-400">
                    {t('notes.lastUpdated')} {new Date(note.updatedAt).toLocaleDateString()}
                  </div>
                </div>
              </CardContent>
              ...
              {showDeleteId === note.id && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
                  <div className="bg-white dark:bg-muted rounded-xl shadow-xl p-6 w-full max-w-xs flex flex-col items-center">
                    ...
                    <div className="font-semibold text-lg mb-1">{t('common.deleteThisNote')}</div>
                    <div className="text-sm text-muted-foreground mb-4 text-center">{t('common.thisActionCannotBeUndone')}</div>
                    <div className="flex gap-3">
                      <Button variant="destructive" onClick={() => deleteNote(note.id)}>{t('common.delete')}</Button>
                      <Button variant="outline" onClick={() => setShowDeleteId(null)}>{t('common.cancel')}</Button>
                    </div>
                  </div>
                </div>
              )}
